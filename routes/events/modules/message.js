        ...
        database.connect().then(async dbClient => {
            ...
            if (upsertResult.modifiedCount === 1 || upsertResult.upsertedCount === 1) {
                ...
                let params = {
                    token: process.env.SLACK_ACCESS_TOKEN,
                    channel: channel,
                    text: ':thumbsup: Point added to ' + thing.trim() + '.  It now has ' + updatedUser.score + ' points.',
                };
                await module.exports.post(params);
            }
        })
        ...
        database.connect().then(async dbClient => {
            ...
            if (upsertResult.modifiedCount === 1) {
                ...
                let params = {
                    token: process.env.SLACK_ACCESS_TOKEN,
                    channel: channel,
                    text: ':thumbsdown: Point removed from ' + thing.trim() + '.  It now has ' + updatedUser.score + ' points.',
                };
                await module.exports.post(params);
            }
        })
